...
@final
@example
<a href="#" id="limit" data-type="limit" data-pk="1">awesome</a>
<script>
$(function(){
    $('#limit').editable({
        url: '/post',
...
(function ($) {
    ...
    var Limit = function (options) {
			this.init('limit', options, Limit.defaults);
    };

    //inherit from Abstract input
    $.fn.editableutils.inherit(Limit, $.fn.editabletypes.abstractinput);

    $.extend(Limit.prototype, {
        /**
        ...
        **/
        value2html: function(value, element) {
					...
					if (value.min == '' && value.max == ''){
						...
						s = '<span>Max. ' + value.max + '</span>';
					} else if (value.max == ''){
						s = '<span>Min. ' + value.min + '</span>';
					} else {
						s = '<span>'
							+ value.min + ' - ' + value.max
							+ '</span>'
		...
		// edit interface
    Limit.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
			tpl:
					'<div class="editable-limit">'
						+ '<table>'
							+ '<tr>'
								+ '<td>Minimum:</td>'
