...
scheduler.render_view_data = function(evs, hold) {
	...
	if (this.config.multi_day && !this._table_view) {
		...
		//var arr =  this._els['dhx_multi_day'];
		//if (arr && arr.length){ // added by alantypoon 20160913
		//  this._rendered_location = arr[0];
		//}
	...
	if (this._table_view) {
		if (hold)
			...
			if (evl.rows) {
				...
				//	h[0] = -1;
				// edited by alantypoon 20160913
				//var arr = this._els["dhx_multi_day"];
				//if (!evs.length && arr && arr.length && arr[0].style.visibility == "visible")
				if (!evs.length && this._els["dhx_multi_day"][0].style.visibility == "visible")
				{
				    h[0] = -1;
				}
