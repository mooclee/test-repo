  ...
  var Resumable = function(opts){
    ...
    $.defaults = {
      ...
      totalSizeParameterName: 'resumableTotalSize',
			fileMimeParameterName: 'resumableFileMime',
    ...
    var appendFilesFromFileList = function(fileList, event){
      ...
      $h.each(fileList, function(file){
        ...
        function addFile(uniqueIdentifier){
          ...
          // prevent added again error
          // added by alantypoon
          var file2 = $.getFromUniqueIdentifier(uniqueIdentifier);
          if (file2){
           $.removeFile(file2);
          }
    ...
    function ResumableChunk(resumableObj, fileObj, offset, callback){
      ...
      $.send = function(){
        ...
        var doneHandler = function(e){
					...
					//type = 'timeout';	// testing				
				
					//var s = type + '\r\n' + vobj(e); console.debug(e); //alert(s);
					if (type == 'timeout'){
					
						// added by alantypoon 20161030
						if (!fileObj.error){
							//alert('Timeout: please upload by another browser.');
							fileObj.error = 'upload error';
							//debugger;
							fileObj.resumableObj.fire('fileerror', fileObj, 'Timeout: please upload by another browser.');
							//$.fire('error', 'Timeout: please upload by another browser.');
						}
					...
					} else 
					{
						...
						if (status == 'success' || status == 'error') {
							...
							///////////////////////////////////////////////////////////////////////////////////
							// svrop.php and resumablejs communication
							// get error and media_id from the server
							// and attach to the resumablefile object
							// added by alantypoon 20161026				
							///////////////////////////////////////////////////////////////////////////////////
							var output = 0, error = 0, media_id = 0;
							
							// READ AT RISK
							try {
								output = JSON.parse(e.srcElement.response);
							} catch (exception){
								console.error(e.srcElement.response);
							}
							
							//console.debug(output, error);
							
							// EXTRA HANDLING
							if (output){
								if (output.error){
									console.error(output.error);
									fileObj.error = output.error;
								} else if (!fileObj.media && output.media && output.media.media_id){
									console.debug('resummable:doneHandler', 'media_id='+output.media.media_id);	// alantypoon 20161026
									fileObj.media = output.media;
								}
							}
        ...
        query[$.getOpt('fileMimeParameterName')] = $.fileObj.file.type;
        // Mix in custom data
        var customQuery = $.getOpt('query');
        if(typeof customQuery == 'function') customQuery = customQuery($.fileObj, $);
        $h.each(customQuery, function(k,v){
    ...
    $.uploadNextChunk = function(){
      ...
      if(!outstanding) {
         console.debug('file complete', this.uid);
